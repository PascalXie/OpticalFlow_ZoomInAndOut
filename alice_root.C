...
int alice_root()
{
	...
	*/
	...
	float X = 1.*nRow/2.+1.;
	float Y = 1.*nCol/2.+1.;
	float r_max = sqrt(X*X+Y*Y);

	h1 = new TH2F("","",nCol*2, -180, 180, nCol, 0., r_max);
	h2 = new TH2F("","",nCol, 0., nCol, nRow, 0., nRow);

	float img1_pole[imgPixelNumber];
	float img2_pole[imgPixelNumber];
...
void toPole(float *img, float *imgPole)
{
	...
	for(int j=0; j<cols; j++)
	{
		...
		h1 -> Fill((theta)/PI*180., r, img[i*cols+j]);
		h2 -> Fill(j, i, img[i*cols+j]);
	}
